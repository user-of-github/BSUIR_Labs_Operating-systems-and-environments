...
int main()
{
	...
	if (inet_pton(AF_INET, kIpAddress, &server_address.sin_addr) <= 0)
	...
	while (true)
	{
		std::cout << "[Client] Type a command to send to server (type 'exit' to finish):" << '\n';
		std::string command{};
		...
		std::cout << "[Client] Received message from server: " << buffer << '\n' << '\n';
	}
